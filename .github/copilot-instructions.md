...
Between the veil and the portrait sits `div.hero-energy`, a blue plasma band (`assets/img/backgrounds/hero-energy.webp`
plus a 900 px version, both straight-alpha WebP generated by `tools/hero-energy/make_energy.py`). On two-column screens
`.hero-section::before` is lowered to `z-index: -3` and `.hero-bg-slides` lifted to `-2`, so the portrait occludes the
band; the layer itself is `-3` after the veil in tree order. Two copies of the same file drift with CSS keyframes
(`transform`/`opacity` only, static placement via `translate`), GSAP adds a scroll parallax on the whole layer
(`initHeroEnergy()`), and `prefers-reduced-motion` switches all of it off. Strength lives in `--hero-energy-opacity`,
the mask on `.hero-energy` caps it to 22 % under the text column (keeps AAA contrast on the copy). Phones get a 1x1
gif source and `display: none`; do not add the band to the mobile hero and do not put a blend mode on it.
