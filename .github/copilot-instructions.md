...
`var(--theme-gradient-start)` (highlight, luminance mask), so the colour follows `[data-project-theme]` with the
usual `--theme-transition`. The section has `isolation: isolate` so the band's `z-index: -1` stays above its own
